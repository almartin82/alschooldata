# ==============================================================================
# LIVE Pipeline Tests for alschooldata
# ==============================================================================
#
...
test_that("Alabama DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Alabama DOE URL and verify HTTP 200")
})
...
test_that("Can download Alabama enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Alabama enrollment file", {
  skip_if_offline()
...
test_that("alschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- alschooldata::get_available_years()
  ...
  tryCatch({
    raw <- alschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- alschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- alschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- alschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- alschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- alschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- alschooldata:::get_cache_path(2024, "enrollment")
